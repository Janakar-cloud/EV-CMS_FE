...
import adminStationsService, { type StationDetail } from '@/lib/admin-stations-service';
...
export default function AdminStationDetailPage() {
  ...
  const loadStation = async () => {
    try {
      setLoading(true);
      const data = await adminStationsService.getStation(params?.id as string);
      setStation(data);
    } catch (error: any) {
  ...
  const handleDecision = async (approve: boolean) => {
    ...
    try {
      setUpdating(true);
      await adminStationsService.verifyStation(station.id, approve);
        ...
        <div className="flex gap-2">
          ...
          {station.status === 'pending' ? (
            <>
              ...
              <Button variant="default" disabled={updating} onClick={() => handleDecision(true)}>
                {updating ? (
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ) : (
