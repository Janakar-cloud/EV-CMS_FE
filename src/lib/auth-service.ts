...
// API client wrapper that delegates to the shared axios-based apiClient
const apiClient = async (endpoint: string, options: RequestInit = {}) => {
  ...
  switch (method) {
    ...
    case 'POST':
      return apiClientInstance.post(endpoint, body);
    case 'PUT':
      return apiClientInstance.put(endpoint, body);
    case 'PATCH':
      return apiClientInstance.patch(endpoint, body);
    case 'DELETE':
      return apiClientInstance.delete(endpoint);
    default:
      return apiClientInstance.get(endpoint);
  }
};
...
const authService = {
  ...
  login: async (credentials: LoginCredentials): Promise<AuthResponse> => {
    try {
      ...
      if (!isEmail) {
        throw new Error(
          'Backend currently supports email/password login only. Please enter your email address.'
        );
      }
      ...
      if (response?.success === false) {
        return {
          ...
          errors: response?.errors || [
            { field: 'general', message: response?.message || 'Login failed. Please try again.' },
          ],
        };
      }
